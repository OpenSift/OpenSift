...
def test_library_page_has_expected_controls(monkeypatch) -> None:
    ...
    assert 'id="selectAllPage"' in html
    assert 'id="batchMoveBtn"' in html
    assert 'id="batchTagBtn"' in html
    assert 'id="batchDeleteBtn"' in html
    assert 'id="batchExportBtn"' in html
